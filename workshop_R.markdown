...
If you're struggling to grasp that, that's okay, we will create a project and see this in action now. To create a new project, in R studio, go to **File > New Project > Version control > Git**. Copy and paste the repository url (we've shared it in the Teams chat for the session) and then set Create project as a subdirectory of: to C:/Users/<username>/repos (you'll need to create that directory assuming you don't already have it). Click 'Create project'.
